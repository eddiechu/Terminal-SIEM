...
## :bookmark:  **Crawl the syslog file and parse it to a standard schema using a delimiter of "|"**
...
For example extract "source IP" from the syslog entries\
...
```
> [!TIP]
> Avoid using regular expressions like `grep -oP 'utmaction="\K[^" ]+'` due to inefficiency
...
## :bookmark:  **Parse new syslogs since last check**
```bash
...
This approach allows you to schedule the task, track the last check position, and avoid overlapping log entries
